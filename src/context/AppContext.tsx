...
type OrdemServico = {
  id: string;
  numero: number; // opcional, se você quiser ter um número de ordem
  empresa_id: string;
  cliente_id: string;
  moto_id: string;
...
type AppContextType = {
  ...
  carregando: boolean;
  ordemsServico?: OrdemServico[]; // opcional, pode ser carregado em outra parte do app
...
export const AppProvider = ({ children }: AppProviderProps) => {
  ...
  const carregarSessao = async () => {
    ...
    // Busca Ordem de Serviço associadas a Empresa
    ...
    if (erroOs || !osData) {
      console.error("OS não encontrados", erroOs);
      setOrdemsServico([]);
      setCarregando(false);
      return;
    }
